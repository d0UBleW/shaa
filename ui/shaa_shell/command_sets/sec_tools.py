...
class sec_tools_action_cmd(CommandSet):
    ...
    def sec_tools_action_info(self: CommandSet, ns: argparse.Namespace):
        ...
        data_list.append(["enabled", role_sec_tools.get_enabled(ns.action)])
        ...
        if "vars" in task.keys() and task["vars"] is not None:
            ...
            for var_key, var in task["vars"].items():
                ...
                user_val = role_sec_tools.get_var(ns.action, var_key)
...
better tab completion"""
    ...
    def sec_tools_set(self: CommandSet, ns: argparse.Namespace):
        ...
        if ns.node_name is not None:
            ...
            role_sec_tools.set_var(action, opt_key, val)
