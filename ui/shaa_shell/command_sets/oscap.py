...
class oscap_action_cmd(CommandSet):
    ...
    def oscap_action_enable(self: CommandSet, ns: argparse.Namespace):
        ...
        for arg_act in ns.action:
            ...
            if arg_act == "all":
                for act in role_oscap.actions.keys():
                    role_oscap.set_enabled(act, True)
                    if ns.verbose:
                        self._cmd.pfeedback(f"[+] {act} enabled successfully")
            else:
                role_oscap.set_enabled(arg_act, True)
                if ns.verbose:
    ...
    def oscap_action_disable(self: CommandSet, ns: argparse.Namespace):
        ...
        for arg_act in ns.action:
            ...
            if arg_act == "all":
                for act in role_oscap.actions.keys():
                    role_oscap.set_enabled(act, False)
                    if ns.verbose:
                        self._cmd.pfeedback(f"[+] {act} disabled successfully")
            else:
                role_oscap.set_enabled(arg_act, False)
                if ns.verbose:
                    self._cmd.pfeedback(f"[+] {act} disabled successfully")
        self._cmd._oscap_has_changed = True  # type: ignore[attr-defined]
    ...
    def oscap_action_info(self: CommandSet, ns: argparse.Namespace):
        ...
        data_list.append(["enabled", role_oscap.get_enabled(ns.action)])
        ...
        if "vars" in task.keys() and task["vars"] is not None:
            ...
            for var_key, var in task["vars"].items():
                ...
                user_val = role_oscap.get_var(ns.action, var_key)
...
class oscap_set_cmd(CommandSet):
    ...
    def _choices_oscap_option_key(
        ...
        if action not in role_oscap.actions.keys():
...
better tab completion"""
    ...
    def oscap_set(self: CommandSet, ns: argparse.Namespace):
        ...
        if ns.node_name is not None:
            ...
            old_value = role_oscap.get_var(action, opt_key)
            role_oscap.set_var(action, opt_key, val)
            self._cmd._oscap_has_changed = True  # type: ignore[attr-defined]
            if isinstance(old_value, TaggedScalar):
                old_value = vault.load(old_value)
            if isinstance(val, TaggedScalar):
    ...
    def oscap_unset(self: CommandSet, ns: argparse.Namespace):
        ...
        if ns.node_name is not None:
            ...
            old_value = role_oscap.get_var(action, opt_key)
            role_oscap.set_var(action, opt_key, None)
            self._cmd._oscap_has_changed = True  # type: ignore[attr-defined]
